...
describe("substack listItems", () => {
  ...
  it("parses RSS items and returns PluginAS2Object array", async () => {
    const fetchFn = makeFetch(SAMPLE_RSS_XML);
    ...
    expect(item.summary).toBe("First post content.");
  });
  ...
  it("strips HTML tags from description to produce plain text", async () => {
    ...
    expect(items[0]!.content).toBe("<p>First post content.</p>");
    expect(items[0]!.summary).toBe("First post content.");
  });
});
...
<rss version="2.0"><channel><title>Empty Feed</title></channel></rss>`;
    const fetchFn = vi.fn().mockResolvedValueOnce({ ok: true, text: async () => emptyFeed } as unknown as Response);
    const items = await substackRssPlugin.listItems("https://someblog.substack.com", fetchFn);
    expect(items).toHaveLength(0);
  });

  it("uses shared RSS 2.0 fixture and parses correctly", async () => {
    const fetchFn = vi.fn().mockResolvedValueOnce({ ok: true, text: async () => SAMPLE_RSS2_XML } as unknown as Response);
    const items = await substackRssPlugin.listItems("https://someblog.substack.com", fetchFn);
    expect(items).toHaveLength(1);
    expect(items[0]!.name).toBe("Test Article");
  });
});
