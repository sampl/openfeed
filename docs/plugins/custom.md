...
If a plugin throws a plain `Error`, OpenFeed records it with code `unknown`. Switching to `FeedError` only adds the structured code.
...
We encourage you to share your plugins by opening a PR to add them to the built-in plugin list.
